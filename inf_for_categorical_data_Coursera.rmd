...
**Ans:** individuals are independent in the pools, number of people in the pools is less than 10% of population, success-failure rate, 5% * 1002 > 10 and 95% * 1002 > 10, so we can apply theoritical method for 95% confidence interval
...
margin of error help quantify the true population proportion for a given confidence level using information collected from the poll . We are able to get a proportion from this pool which at this point, only represents the opinion of people being polled. If we were to ask another group of people, we would likely get a proportion that is different from the one we get. The different proportion we would get when we ask different group of people is called sampling error. however, we are not interestedi in the opinion of people who take the poll, we are interested in the opinion of general public. In order to do that, we need a way that allows us to use sample data, knowing existance of sampling error, to make scientific inference to the general public. margin of error help quantify the range where the true population falls in given our sample data for a given confidence level with sampling error in mind. in this data, our sample data indicates 5% of Americans identified themselves as atheist, and knowing the margin of error as 1.34%, we can infer the proportion of atheist for the general public in US is about plus/minus 1.35% to the 5%.
...
inf = inference(y = response, data = us12, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")
a = 0.0634-0.0364
a/2
```
...
inference(y = response, x = as.factor(year), data = sp, statistic = "proportion", type = "ht", method = "theoretical", alternative = "twosided", success = "atheist", null = 0)
...
inference(y = response, x = as.factor(year), data = us, statistic = "proportion", type = "ht", method = "theoretical", alternative = "twosided", success = "atheist", null = 0)
```
