...
ggplot(samp, aes(x=area)) + 
  geom_histogram(binwidth = 100, # You can adjust the bin width as needed
                 color="black", fill="blue") +
  labs(title="Histogram of Area", x="Area", y="Count") +
  theme_minimal()

# Assuming your data frame is named Samp and the column is named area

# Calculate mean
mean_area <- mean(samp$area)

# Calculate median
median_area <- median(samp$area)

# Calculate standard deviation (sample standard deviation)
# Since you mentioned the sample size is 60, we assume that it is less than the population size
# and therefore set the flag to TRUE for sample standard deviation
sd_area <- sd(samp$area, na.rm = TRUE)

# Print the summary statistics
summary_stats <- data.frame(Mean = mean_area, Median = median_area, Standard_Deviation = sd_area)
print(summary_stats)
...
# type your code for the Exercise here, and Knit
# yes, 1500 is within the CI [1354,1629]
```
...
```{r plot-99-perc-cis}
# type your code for the Exercise here, and Knit
#(x_bar - Z_95*ME - X_bar) * (Z_99/z_95) + x_bar
ci<- ci %>%
  mutate(lower_99 = ci$mean + (ci$lower-ci$mean) * (z_star_99/z_star_95),
         upper_99 = ci$mean + (ci$upper-ci$mean) * (z_star_99/z_star_95))  %>%
  mutate(capture_mu = ifelse(lower_99 < params$mu & upper_99 > params$mu, "yes", "no"))
ci_data <- data.frame(ci_id = c(1:50, 1:50),
                      ci_bounds_99 = c(ci$lower_99, ci$upper_99),
                      capture_mu = c(ci$capture_mu, ci$capture_mu))

ggplot(data = ci_data, aes(x = ci_bounds_99, y = ci_id, 
                           group = ci_id, color = capture_mu)) +
  geom_point(size = 2) +  # add points at the ends, size = 2
  geom_line() +           # connect with lines
  geom_vline(xintercept = params$mu, color = "darkgray") # draw vertical line
