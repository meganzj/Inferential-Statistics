...
90% confidence interval would be narrower than the 99% CI.
...
require(dplyr)
nc %>% group_by(mature) %>% summarise(max_age = max(mage),
                                      min_age = min(mage))
```
...
My research question is number of hospital visits during pregnancy is different for premature and full-term birth at the first 34 weeks. The dataset may not be the best as i would need only the first 34 weeks of visit.
another research question is if the weight gain by monther during pregancy in pounds is different between lowbirthweight baby and regular weight baby.
y would be gained
x would be lowbirthweight
null hypothesis: weight gain by mother during pregancy is not different between low birth weight baby and regular weight baby.
alternative hypothesis: weight gain by monther during pregancy is different.

Step 1. Check conditions for two sample ttest
normality: both sample distribution did not show extreme skew and outliers, based on the decent sample size, CLT should be able to support that the sampling distribution is approaching normal shape. Given the unknown population variance for both group, we shoud use t test for two independent population mean for our research topic. 
constant variance: since the sample size is relative large, s would be a good estimate to sigma, hence, the population variance should be relative close to each other.
independence: it is random sample and 1,000 cases is likely less than 10% of population. 

With all 3 conditions met, using two sample ttest sounds reasonable.

Step 2. Set our significance level to 0.05

Step 3. Run hypothesis test and analyze results.
based on the following ttest, the data show strong evidence in favor of alternative hypothesis that weight gain in mother during pregency is different between low birth weight baby and regular weight baby.

```{r pick-your-own}
# type your code for the Exercise here, and Knit
inference(y = gained, x = lowbirthweight, data = nc, type = 'ht', null = 0, statistic = 'mean', method = 'theoretical', alternative = 'twosided')
